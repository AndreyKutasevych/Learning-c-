...
namespace Hello_world
{
    public class HumanFactory: BaseShop
    {
        ...
        public HumanFactory(ConsoleTextViewer textViewer, Action menuStartAction):base(menuStartAction)
        {
            ...
            {
                _listSkinColorReturn.Add(new ConsoleTextViewer.ButtonReturning<SkinColor>(color.ToString(), () => color));
            }
            foreach (var ageGroup in _listAgeGroup)
            {
                _listAgeGroupReturn.Add(new ConsoleTextViewer.ButtonReturning<AgeGroup>(ageGroup.ToString(), () => ageGroup));
            }
        }
        ...
        private void Sort()
        {
            foreach (PeopleDescriptor human in _listPeople)
            {
                Console.WriteLine("Here is the full list of humans:");
                Console.WriteLine($"Name: {human.Name}, age: {human.Age}, price: {human.Price}, he is {human.AgeGroup}");
            }
        }
        private void Sort(SkinColor color)
        {
            List<PeopleDescriptor> _listPeopleMatch = new List<PeopleDescriptor>();
            foreach (var human in _listPeople)
            {
